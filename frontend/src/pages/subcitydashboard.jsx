...
const PLAN_FIELDS = [
  { key: "hubannoo_uummuu",  label: "Hubannoo Uummuu",  color: "#0f766e" },
  { key: "horannaa_misensaa",label: "Horannaa Misensaa",color: "#1e40af" },
  { key: "buusi_jirataa",   label: "Buusi Jirataa",    color: "#475569" },
  { key: "buusi_daldalaa",  label: "Buusi Daldalaa",   color: "#64748b" },
];
...
function OverviewPage({ dbPlan, u }) {
  const hasPlan = dbPlan && PLAN_FIELDS.some((f) => Number(dbPlan[f.key] || 0) > 0);
  const totalWeight = dbPlan
    ? WOREDAS.reduce((s, w) => s + Number(dbPlan[`weight_${w.id}`] || 0), 0) : 0;
  const share = (woredaId, total) => {
    if (!dbPlan || totalWeight === 0) return Math.round(total / 4);
    return Math.round((Number(dbPlan[`weight_${woredaId}`] || 0) / totalWeight) * total);
  };
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#1e293b]">Sub-city Overview</h1>
        <p className="text-[#64748b] text-sm mt-0.5">{u.name} — monitoring 4 woredas</p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {WOREDAS.map((w) => (
          <div key={w.id} className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm px-5 py-4 flex flex-col items-center text-center">
      ...
      {hasPlan ? (
        <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-[#e2e8f0]"
            ...
            <p className="text-white/60 text-xs mt-0.5">Fetched from database · Year {dbPlan.year}</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#e2e8f0] px-6 py-10 flex flex-col items-center text-center shadow-sm">
          ...
          <p className="text-[#1e293b] font-semibold mb-1">No Annual Plan Yet</p>
...
function BuusaaPlanPage({ onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSaved(true); setForm({ ...EMPTY_PLAN }); setPopulationTotal("");
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[#64748b] text-sm mt-0.5">Enter subcity totals. The system distributes proportionally by woreda.</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-[#e2e8f0]"
            ...
            <p className="text-white/60 text-xs mt-0.5">Distributed automatically using fixed percentages.</p>
          </div>
          <div className="px-5 py-5">
            <div className="max-w-xs mb-5">
              <label className="block text-sm font-medium text-[#1e293b] mb-1.5">Total</label>
              <input type="number" min="0" value={populationTotal}
                onChange={(e) => setPopulationTotal(e.target.value)} placeholder="e.g. 10000"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {WOREDAS.map((w, i) => {
                ...
                const allocated = populationTotal !== "" && Number(populationTotal) > 0
                  ? Math.round(Number(populationTotal) * WOREDA_PCTS[w.id]) : null;
                return (
                  <div key={w.id}>
                    <label className="block text-xs font-semibold text-[#64748b] mb-1.5">{w.name}</label>
                    <input type="number" readOnly value={allocated !== null ? allocated : ""} placeholder={`${pct}%`}
                    ...
                    <p className="text-xs text-[#94a3b8] mt-1">
                      <span className="font-semibold" style={{ color: colors[i] }}>{pct}%</span> of total
                    </p>
                  </div>
                );
              })}
          ...
          <div className="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
            {PLAN_FIELDS.map(({ key, label, color }) => (
              <div key={key}>
                ...
                <input type="number" min="0" name={key} value={form[key]} onChange={handleField} placeholder="0"
                  className="w-full border border-[#e2e8f0] rounded-lg px-3 py-2.5 text-sm text-[#1e293b] bg-[#f4f6f9] focus:outline-none focus:ring-2 focus:ring-[#1a3a5c]/20 focus:border-[#1a3a5c]" />
              </div>
            ))}
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#f1f5f9]">
                    ...
                    {WOREDAS.map((w) => <th key={w.id} className="text-left px-5 py-3 text-xs font-semibold text-[#64748b] uppercase tracking-wide">{w.name}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {PLAN_FIELDS.map(({ key, label, color }) => {
                    ...
                    return (
                      <tr key={key} className="border-b border-[#f1f5f9] hover:bg-[#f4f6f9] transition-colors">
                        ...
                        <td className="px-5 py-3 font-semibold text-[#1e293b]">{total.toLocaleString()}</td>
                        {WOREDAS.map((w) => <td key={w.id} className="px-5 py-3 text-[#64748b]">{share(w.id, total).toLocaleString()}</td>)}
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <div className="flex items-center justify-between bg-white rounded-xl border border-[#e2e8f0] px-5 py-4">
          <div>
            {saved && <p className="flex items-center gap-2 text-[#166534] text-sm font-semibold"><CheckIcon /> Saved.</p>}
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[#64748b] text-sm mt-0.5">Plan management for {title}</p>
      </div>
      <div className="bg-white rounded-xl border border-[#e2e8f0] px-6 py-14 flex flex-col items-center text-center shadow-sm">
        ...
        <p className="text-[#94a3b8] text-sm max-w-xs">
          Annual plan management for <strong>{title}</strong> will be available here.
        </p>
...
function WorkAnalysisPage({ sector }) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#1e293b]">Work Analysis — {sectorLabel}</h1>
        <p className="text-[#64748b] text-sm mt-0.5">Select a woreda to view performance data.</p>
      </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {WOREDAS.map((w) => (
          <button
            ...
            style={activeWoreda === w.id ? { background: "linear-gradient(90deg,#1a3a5c 0%,#1e4976 100%)" } : {}}
          >
            {w.name}
          </button>
        ))}
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-[#e2e8f0]"
          style={{ background: "linear-gradient(90deg,#1a3a5c 0%,#1e4976 100%)" }}>
          <p className="text-sm font-semibold text-white">{woredaLabel} — {sectorLabel}</p>
          <p className="text-white/60 text-xs mt-0.5">Performance analysis will appear here once connected.</p>
        </div>
        <div className="px-6 py-14 flex flex-col items-center text-center">
          ...
          <p className="text-[#1e293b] font-semibold mb-2">Coming Soon</p>
          <p className="text-[#94a3b8] text-sm max-w-xs">
            Analytics for <strong>{sectorLabel}</strong> · <strong>{woredaLabel}</strong> will be available here.
          </p>
          <span className="mt-4 inline-block bg-[#eef4fb] text-[#1a3a5c] border border-[#dce8f4] text-xs font-semibold px-4 py-2 rounded-full">
            Under Development
          </span>
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[#64748b] text-sm mt-0.5">Select a woreda to view submitted reports.</p>
      </div>
      <div className="flex gap-2 mb-6 flex-wrap">
        {WOREDAS.map((w) => (
          <button key={w.id} onClick={() => setActiveWoreda(w.id)}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
              activeWoreda === w.id ? "text-white shadow" : "bg-white border border-[#e2e8f0] text-[#64748b] hover:border-[#1a3a5c] hover:text-[#1a3a5c]"
            }`}
            style={activeWoreda === w.id ? { background: "linear-gradient(90deg,#1a3a5c 0%,#1e4976 100%)" } : {}}>
            {w.name}
          </button>
      ...
      {hasPlan && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {PLAN_FIELDS.map(({ key, label, color }) => {
            ...
            return (
              <div key={key} className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm px-4 py-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
                  <p className="text-xs font-semibold text-[#64748b] uppercase tracking-wide truncate">{label}</p>
                </div>
                <p className="text-xl font-extrabold text-[#1e293b]">{s.toLocaleString()}</p>
      ...
      <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-[#e2e8f0]"
          ...
          <p className="text-white/60 text-xs mt-0.5">Live data will appear once connected.</p>
        </div>
        <div className="px-6 py-12 flex flex-col items-center text-center">
          ...
          <p className="text-[#94a3b8] text-sm max-w-sm">
            Wire to <code className="bg-[#f4f6f9] px-1 rounded text-xs">/api/reports?woreda={activeWoreda}</code> once backend is ready.
          </p>
        </div>
      </div>
    </div>
...
export default function SubCityDashboard({ user: propUser }) {
  ...
  const renderContent = () => {
    ...
    if (activeNav === "plan") {
      if (!activePlanSector) return (
        <div>
          <h1 className="text-2xl font-bold text-[#1e293b] mb-1">Annual Plan</h1>
          <p className="text-[#64748b] text-sm mb-6">Select a sector from the sidebar.</p>
          <div className="grid grid-cols-2 gap-4">
            {SECTORS.map((s) => (
              <button key={s.id} onClick={() => { setActivePlanSector(s.id); setPlanOpen(true); }}
                ...
                <p className="text-xs text-[#94a3b8] mt-1">{s.id === "buusaa" ? "Active" : "Coming soon"}</p>
              </button>
            ))}
          </div>
        </div>
      );
      if (activePlanSector === "buusaa") return <BuusaaPlanPage onSave={handleSavePlan} />;
      return <ComingSoonPage title={SECTORS.find((s) => s.id === activePlanSector)?.label ?? ""} />;
    }
    if (activeNav === "analysis") {
      if (!activeAnalysisSector) return (
        <div>
          ...
          <div className="bg-white rounded-xl border border-[#e2e8f0] px-6 py-14 flex flex-col items-center text-center shadow-sm">
            ...
            <p className="text-[#94a3b8] text-sm">Choose a sector from the sidebar to view analysis.</p>
          </div>
        </div>
      );
  ...
  return (
    <div className="flex h-screen max-h-screen bg-[#f4f6f9] font-['DM_Sans',system-ui,sans-serif] overflow-hidden"
      ...
      <aside className={`${sideW} flex-shrink-0 flex flex-col transition-all duration-300 overflow-hidden`}
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          ...
          {/* Overview */}
          <button onClick={() => { setActiveNav("overview"); setActivePlanSector(null); setActiveAnalysisSector(null); }}
            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
              activeNav === "overview" ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"
            }`}>
            <GridIcon />
          ...
          <div>
            <button onClick={() => { setPlanOpen((o) => !o); setActiveNav("plan"); setActivePlanSector(null); }}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                isPlanActive ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"
              }`}>
              <TargetIcon />
              {!collapsed && (
                <>
            ...
            {!collapsed && planOpen && (
              <div className="ml-4 pl-2 border-l border-white/10 mt-0.5 space-y-0.5">
                {SECTORS.map((s) => (
                  <button key={s.id}
                    onClick={() => { setActiveNav("plan"); setActivePlanSector(s.id); }}
          ...
          <div>
            <button onClick={() => { setAnalysisOpen((o) => !o); setActiveNav("analysis"); setActiveAnalysisSector(null); }}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                isAnalysisActive ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"
              }`}>
              <AnalysisIcon />
              {!collapsed && (
                <>
                  <span className="flex-1 truncate text-left">Work Analysis</span>
            ...
            {!collapsed && analysisOpen && (
              <div className="ml-4 pl-2 border-l border-white/10 mt-0.5 space-y-0.5">
                {SECTORS.map((s) => (
                  <button key={s.id}
                    onClick={() => { setActiveNav("analysis"); setActiveAnalysisSector(s.id); }}
                    className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
          ...
          {/* Woreda Reports */}
          <button onClick={() => { setActiveNav("reports"); setActivePlanSector(null); setActiveAnalysisSector(null); }}
            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
              activeNav === "reports" ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"
            }`}>
            <ListIcon />
