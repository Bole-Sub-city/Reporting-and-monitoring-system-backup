...
// ─── GaliiSassabuOverviewPlanTable ───────────────────────────────────────────
// Shows Mana Qophessaa Total + Idilee Total targets per woreda + subcity total.
// Fetches per-woreda targets from getWoRedaAnalysis("galii_sassabu", wId, "annual").
function GaliiSassabuOverviewPlanTable({ dbGaliiSassabuPlan }) {
  const ACCENT = "#c2410c";
  ...
  useEffect(() => {
    Promise.all(
      WOREDAS.map((w) =>
        fetchWoRedaAnalysis("galii_sassabu", w.id, "annual")
          .then((d) => [w.id, d.targets || {}])
          .catch(() => [w.id, {}]),
      ),
    ).then((results) => {
  ...
  const fields = [
    {
      ...
      targetKey: "mana_qophessaa_total",
      label: "Mana Qophessaa Total",
      color: "#c2410c",
    },
    {
      key: "idilee_total",
      targetKey: "idilee_total",
      label: "Idilee Total",
      color: "#ea580c",
    },
  ];
  ...
  return (
    <div className="mt-6 bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden">
      ...
      >
        ...
        <p className="text-white/60 text-xs mt-0.5">
          Waggaa {dbGaliiSassabuPlan.year} · Subcity + Aanaalee 4
        </p>
      </div>
      {loading ? (
        <div className="flex items-center justify-center h-16">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#f1f5f9] bg-[#f8fafc]">
                ...
                <th
                  className="text-right px-5 py-3 text-xs font-semibold uppercase tracking-wide bg-[#fff7ed] min-w-[100px]"
                  style={{ color: ACCENT }}
                >
                  Subcity
                </th>
            ...
            <tbody>
              {fields.map(({ key, targetKey, label, color }) => {
                const subcityVal = WOREDAS.reduce(
                  (s, w) => s + Number(woredaTargets[w.id]?.[targetKey] ?? 0),
                  0,
                );
                return (
                  <tr
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right font-semibold text-[#1e293b] bg-[#fff7ed]">
                      {subcityVal.toLocaleString()}
                    </td>
                    {WOREDAS.map((w) => (
                      <td
                      ...
                      >
                        {Number(
                          woredaTargets[w.id]?.[targetKey] ?? 0,
                        ).toLocaleString()}
                      </td>
                    ))}
                  </tr>
  ...
  return (
    <div>
      ...
      <FlatSectorPlanOverview
        ...
        accentColor="#7e22ce"
      />
      {/* ── Galii Sassabu plan — Mana Qophessaa + Idilee with per-woreda columns ── */}
      <GaliiSassabuOverviewPlanTable dbGaliiSassabuPlan={dbGaliiSassabuPlan} />
    </div>
  );
}
...
function GaliiSassabuSubcityPlanPage() {
  ...
  useEffect(() => {
    fetchSubcityGaliiSassabuPlan()
      .then((d) => {
        if (d.plan) {
          setExisting(d.plan);
          // Plan exists if any value > 0
          if (
            Number(d.plan.mana_qophessaa_total || 0) > 0 ||
            Number(d.plan.idilee_total || 0) > 0
          ) {
            setPlanExists(true);
          }
        }
  ...
  // Only allow submit if no plan exists yet, or admin has approved unlock
  const hasValues = WOREDAS.some(
    (w) =>
      Number(woredaForms[w.id].mana_qophessaa_total || 0) > 0 ||
      Number(woredaForms[w.id].idilee_total || 0) > 0,
  );
  const canSubmit = hasValues && (!planExists || unlockStatus === "approved");
  ...
  return (
    <div>
      ...
      <PlanUnlockBanner
        ...
        onStatusChange={setUnlockStatus}
      />
      {existing && (
        <div
...
function GaliiSubcityPlanPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSaveError(
        err?.response?.data?.message || "Failed to save Galii plan.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const mqTotals = Object.fromEntries(
    SC_MANA_QOPHESSAA_SOURCES.flatMap((s) => [
      [
        `mq_${s.key}_kg`,
        WOREDAS_LIST.reduce(
          (sum, w) => sum + Number(woredaForms[w.id][`mq_${s.key}_kg`] || 0),
          0,
        ),
      ],
      [
        ...
        WOREDAS_LIST.reduce(
          (sum, w) =>
            sum + Number(woredaForms[w.id][`mq_${s.key}_qarshii`] || 0),
          0,
        ),
      ],
    ]),
  ...
  const idileeTotals = Object.fromEntries(
    SC_IDILEE_SOURCES.map((s) => [
      ...
      WOREDAS_LIST.reduce(
        (sum, w) =>
          sum + Number(woredaForms[w.id][`idilee_${s.key}_qarshii`] || 0),
        0,
      ),
    ]),
  );
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#f1f5f9] bg-[#f8fafc]">
                  ...
                  <th
                    colSpan={2}
                    className="text-center px-4 py-3 text-xs font-semibold text-[#0f172a] uppercase bg-[#eff6ff] min-w-[140px]"
                  >
                    Waliigala
                  </th>
                </tr>
                <tr className="border-b border-[#f1f5f9] bg-[#f8fafc]">
                  ...
                  {WOREDAS_LIST.map((w) => (
                    <>
                      <th
                        key={`${w.id}-kg`}
                        className="px-3 py-2 text-xs font-semibold text-[#64748b] text-right border-l border-[#f1f5f9]"
                      >
                        KG
                      </th>
                      <th
                        key={`${w.id}-q`}
                        className="px-3 py-2 text-xs font-semibold text-[#64748b] text-right"
                      >
                        Qarshii
                      </th>
                    </>
                  ))}
                  <th className="px-3 py-2 text-xs font-semibold text-[#0f172a] text-right bg-[#eff6ff] border-l border-[#f1f5f9]">
                    KG
                  </th>
                  <th className="px-3 py-2 text-xs font-semibold text-[#0f172a] text-right bg-[#eff6ff]">
                    Qarshii
                  </th>
                </tr>
              </thead>
                  ...
                  >
                    ...
                    {WOREDAS_LIST.map((w) => (
                      <>
                        <td
                          key={`${w.id}-kg`}
                          className="px-2 py-1.5 border-l border-[#f1f5f9]"
                        >
                          <input
                            type="number"
                            min="0"
              ...
              <thead>
                <tr className="border-b border-[#f1f5f9] bg-[#f8fafc]">
                  ...
                  {WOREDAS_LIST.map((w) => (
                    <th
                      key={w.id}
                      className="text-right px-4 py-3 text-xs font-semibold text-[#64748b] uppercase min-w-[130px]"
                    >
                      {w.name}
                    </th>
                  ))}
                    ...
                    {WOREDAS_LIST.map((w) => (
                      <td key={w.id} className="px-4 py-2">
                        <input
                          ...
                          step="0.01"
                          value={
                            woredaForms[w.id][`idilee_${src.key}_qarshii`] ?? ""
                          }
                          onChange={(e) =>
                            handleField(
                              w.id,
                              `idilee_${src.key}_qarshii`,
                              e.target.value,
                            )
                          }
                          placeholder="0.00"
                  ...
                  {WOREDAS_LIST.map((w) => {
                    const wTotal = SC_IDILEE_SOURCES.reduce(
                      (sum, s) =>
                        sum +
                        Number(
                          woredaForms[w.id][`idilee_${s.key}_qarshii`] || 0,
                        ),
                      0,
                    );
                    return (
                      <td
                        key={w.id}
                        className="px-4 py-2 text-right text-[#1e40af]"
                      >
              ...
              {renderGroupedFieldRows(
                ...
                (key) => {
                  ...
                  if (
                    ...
                    cfg.fields.some(
                      (f) =>
                        f._parent === field._parent && f.key.endsWith("_dhi"),
                    )
                  ) {
                    const group = cfg.fields.filter(
                      (f) => f._parent === field._parent,
                    );
                    return group.reduce(
                      (acc, gf) =>
                ...
                // Per-sub-row data cells: Target | Submitted | % Done
                (key) => {
                  ...
                  const pct =
                    periodTarget > 0
                      ...
                      : 0;
                  return [
                    <td key="target" className="px-5 py-3 text-[#64748b]">
...
// ─── Helper: render analysis table body rows with Carraa Hojii parent grouping ─
...
function renderGroupedFieldRows(
  ...
  while (i < fields.length) {
    const f = fields[i];
    const isGrouped = (f._totalSubs ?? 1) > 1;
    ...
    if (!isGrouped) {
      ...
      const group = [];
      while (i < fields.length && fields[i]._parent === parentKey) {
        group.push(fields[i]);
        i++;
      }
                  ...
                  if (hasDhiDub) {
                    ...
                    return WOREDAS.reduce((sum, w) => {
                      ...
                      return (
                        sum +
                        group.reduce(
                          (acc, gf) => acc + Number(wd?.actuals?.[gf.key] ?? 0),
                          0,
                        )
                      );
                    }, 0);
                            ...
                            {renderGroupedFieldRows(
                              ...
                              (key) => {
                                ...
                                return [
                                  ...
                                  <td
                                    key="s"
                                    className="px-4 py-3 font-semibold text-[#1e293b]"
                                  >
                                    {actual.toLocaleString()}
                                  </td>,
                                  <td key="pct" className="px-4 py-3">
                                    <span
...
function GenericSubcityAnalysisPage({ sector }) {
  ...
  // For display: subcity totals overview cards.
  // For buusaa fixed-per-woreda fields, sum across all 4 woreda suffixes.
  const FIXED_KEYS = new Set(
    sector === "buusaa" ? FIXED_WEREDA_FIELDS.map((f) => f.key) : [],
  );
  const getPlanTotal = (fieldKey) => {
    if (!plan) return 0;
    if (FIXED_KEYS.has(fieldKey)) {
      return WOREDAS.reduce(
        (s, w) => s + Number(plan[`${fieldKey}_${w.id}`] || 0),
        0,
      );
    }
    return Number(plan[fieldKey] || 0);
  };
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-[#e2e8f0] shadow-sm overflow-hidden mb-6">
            ...
            <div className="px-5 py-4 space-y-3">
              {/* Group Dhi+Dub fields, show combined total; Int and plain fields shown individually */}
              {(() => {
                const items = [];
                let i = 0;
                while (i < cfg.fields.length) {
                  ...
                  const hasDhiDub =
                    f._totalSubs > 1 &&
                    cfg.fields.some(
                      (x) => x._parent === f._parent && x.key.endsWith("_dhi"),
                    );
                  if (hasDhiDub) {
                    // Collect group
                    const group = [];
                    while (
                      i < cfg.fields.length &&
                      cfg.fields[i]._parent === f._parent
                    ) {
                      group.push(cfg.fields[i]);
                      i++;
                    }
                    // Combined total card
                    const combinedTotal = group.reduce(
                      (acc, gf) => acc + getPlanTotal(gf.key),
                      0,
                    );
                    items.push(
                      <div
                        key={f._parent}
                        className="bg-[#eff6ff] rounded-xl border border-[#bfdbfe] px-4 py-3"
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <span
                            ...
                            style={{ backgroundColor: f.color }}
                          />
                          <p className="text-xs font-bold text-[#1e40af] uppercase tracking-wide">
                            {f._parentLabel}
                          </p>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          {group.map((gf) => (
                            <div
                              key={gf.key}
                              className="text-center bg-white rounded-lg px-2 py-2 border border-[#e2e8f0]"
                            >
                              <p className="text-[9px] font-bold text-[#1e40af] uppercase mb-1">
                                {gf._subLabel}
                              </p>
...
export default function SubCityDashboard({ user: propUser }) {
  ...
  const renderContent = () => {
    ...
    if (activeNav === "daldala_a_submit") return <DaldalASubmitForm />;
    if (activeNav === "daldala_a_analysis") return <DaldalAAnalysisPage />;
    ...
    >
      ...
      >
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          ...
          {/* ── Daldala A (subcity-only) ── */}
          <button
            onClick={() => {
              setActiveNav("daldala_a_submit");
              setActivePlanSector(null);
              setActiveAnalysisSector(null);
            }}
            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
              activeNav === "daldala_a_submit"
                ? "bg-white/15 text-white"
                : "text-white/60 hover:bg-white/10 hover:text-white"
            }`}
          >
            ...
            {!collapsed && <span className="truncate">Daldala A</span>}
          </button>
          <button
            onClick={() => {
              setActiveNav("daldala_a_analysis");
              setActivePlanSector(null);
              setActiveAnalysisSector(null);
            }}
            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
              activeNav === "daldala_a_analysis"
                ? "bg-white/15 text-white"
                : "text-white/60 hover:bg-white/10 hover:text-white"
            }`}
          >
            <AnalysisIcon />
            {!collapsed && <span className="truncate">Daldala A Analysis</span>}
          </button>
        ...
        <header className="bg-white border-b border-[#e2e8f0] px-6 py-3 flex items-center justify-between flex-shrink-0">
          <h2 className="text-[#1e293b] font-semibold text-base">
            {{
              ...
              daldala_a_submit: "Daldala A – Submit",
              daldala_a_analysis: "Daldala A – Work Analysis",
