...
function ChevronIcon({ open }) {
  return (
    <svg className="w-4 h-4 flex-shrink-0 transition-transform duration-200" style={{ transform: open ? "rotate(90deg)" : "rotate(0deg)" }}
...
const BUUSAA_FIELDS = [
  ...
  { name: "hojiiwwanMootummaa",       label: "Hojiiwwan Mootummaa",         required: true,  type: "number" },
  { name: "buuusiJirataa",            label: "Bu'uusi Jirataa",             required: true,  type: "number" },
  { name: "buuusiDaldalaa",           label: "Bu'uusi Daldalaa",            required: true,  type: "number" },
  { name: "buuusiDaldalaaFiGumaataa", label: "Bu'uusi Daldalaa fi Gumaataa",required: true,  type: "number" },
  { name: "gumaataMootummaa",         label: "Gumaata Mootummaa",           required: true,  type: "number" },
...
const WORKS = [
  ...
  { id: "revenue",  label: "Revenue",        icon: RevenueIcon,  color: "bg-green-100 text-green-600"  },
  { id: "work1",    label: "Work Area 1",    icon: WorkAreaIcon, color: "bg-blue-100 text-blue-600"    },
  { id: "work2",    label: "Work Area 2",    icon: WorkAreaIcon, color: "bg-amber-100 text-amber-600"  },
];
    ...
    <div>
      ...
      <div className={`bg-white rounded-xl border border-gray-200 px-6 py-12 flex flex-col items-center justify-center text-center`}>
        ...
        <p className="text-gray-400 text-sm max-w-sm mb-6">
          The submission form for <strong>{title}</strong> is under development.<br />
    ...
    <div>
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Complete all required fields and submit before the deadline</p>
        </div>
        <button className="flex items-center gap-2 border border-gray-300 text-gray-600
      ...
      <div className="bg-white rounded-xl border border-gray-200 px-5 py-4 mb-5
        ...
        <div className="text-right flex-shrink-0">
          ...
          <p className="text-amber-600 text-xs mt-0.5">⏰ Deadline: 5:00 PM today</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-5">
          ...
          <div className="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
            {BUUSAA_FIELDS.map(({ name, label, required, type, fullWidth }) => (
              <div key={name} className={fullWidth ? "sm:col-span-2 sm:w-1/2" : ""}>
                ...
                <input type={type} name={name} value={form[name] ?? ""} onChange={handleField}
                  required={required} placeholder="0" min="0"
                  className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm
            ...
            <div className="sm:col-span-2">
              ...
              <textarea value={yaada} onChange={(e) => setYaada(e.target.value)}
                placeholder="Enter Yaada Gudinaa" rows={4}
                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm
                           text-gray-800 bg-gray-50 focus:outline-none focus:ring-2
                           focus:ring-purple-300 focus:border-transparent
...
export default function WoRedaDashboard({ user }) {
  const u = user || { name: "Biruk Haile", woreda: "Woreda 08", subcity: "Kirkos", initials: "BH" };
  ...
  return (
    <div className="flex h-screen bg-[#f0f2f5] font-['DM_Sans',system-ui,sans-serif] overflow-hidden">
      ...
      <aside className={`${sideW} flex-shrink-0 flex flex-col transition-all duration-300 overflow-hidden`}
        ...
        <div className="flex items-center gap-3 px-4 py-4 border-b border-white/10 flex-shrink-0">
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              <p className="text-white font-bold text-sm leading-tight truncate">{u.woreda}</p>
            ...
            {!collapsed && worksOpen && (
              <div className="ml-4 border-l border-white/10 pl-2 py-1 space-y-0.5">
                {WORKS.map(({ id, label, icon: Icon }) => {
                  ...
                  return (
                    <button key={id}
                      onClick={() => { setActiveNav("works"); setActiveWork(id); }}
        ...
        <div className="border-t border-white/10 py-2 flex-shrink-0">
          <button className="w-full flex items-center gap-3 px-4 py-2.5 text-white/60
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-left hidden sm:block">
                <p className="text-gray-800 text-sm font-semibold leading-tight">{u.name}</p>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6">
          ...
          {activeNav === "dashboard" && (
            <div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                  { label: "Total Submitted", value: "24", color: "bg-purple-50 border-purple-200 text-purple-700" },
                  { label: "Pending Review",  value: "3",  color: "bg-amber-50 border-amber-200 text-amber-700"   },
                  { label: "Approved",        value: "19", color: "bg-green-50 border-green-200 text-green-700"   },
                  { label: "Rejected",        value: "2",  color: "bg-red-50 border-red-200 text-red-700"         },
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {WORKS.map(({ id, label, icon: Icon, color }) => (
                  <button key={id} onClick={() => { setActiveNav("works"); setActiveWork(id); setWorksOpen(true); }}
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {[
                      { date: "2025-07-15", section: "Buusaa Gonofaa", type: "Gabaasa Guyyaa", status: "Approved" },
                      { date: "2025-07-14", section: "Revenue",        type: "Gabaasa Guyyaa", status: "Approved" },
                      { date: "2025-07-13", section: "Buusaa Gonofaa", type: "Gabaasa Guyyaa", status: "Pending"  },
                      { date: "2025-07-07", section: "Buusaa Gonofaa", type: "Gabaasa Torban", status: "Approved" },
                    ].map((r, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        <td className="px-5 py-3 text-gray-700">{r.date}</td>
                        <td className="px-5 py-3 text-gray-700">{r.section}</td>
                        ...
                        <td className="px-5 py-3">
                          <button className="text-purple-600 hover:underline text-xs font-medium">View</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {activeNav === "works" && (
            <>
              ...
              {/* Revenue — placeholder */}
              {activeWork === "revenue" && (
                <PlaceholderSubmit title="Revenue" color="bg-green-100 text-green-600"
                  icon={RevenueIcon} u={u} onBack={() => setActiveWork(null)} />
              )}

              {/* Work Area 1 — placeholder */}
              {activeWork === "work1" && (
                <PlaceholderSubmit title="Work Area 1" color="bg-blue-100 text-blue-600"
                  icon={WorkAreaIcon} u={u} onBack={() => setActiveWork(null)} />
              )}

              {/* Work Area 2 — placeholder */}
              {activeWork === "work2" && (
                <PlaceholderSubmit title="Work Area 2" color="bg-amber-100 text-amber-600"
                  icon={WorkAreaIcon} u={u} onBack={() => setActiveWork(null)} />
              )}
            </>
          )}
          ...
          {activeNav === "announcements" && (
            <div>
              ...
              <div className="space-y-4">
                {[
                  { title: "Monthly report deadline extended", date: "July 14, 2025", body: "The deadline for monthly reports has been extended to July 20, 2025." },
                  { title: "New reporting fields added",       date: "July 10, 2025", body: "Zayitii and Sukkaara fields have been added to the daily report form." },
                ].map((a, i) => (
              ...
              <div className="bg-white rounded-xl border border-gray-200 px-6 py-6 max-w-lg">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">{u.subcity} &middot; {u.woreda}</p>
                  </div>
                </div>
                <div className="space-y-4">
                  {[
